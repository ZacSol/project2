...
{  let numberOfDays = this.getDaysInMonth();
//This will be the starting day to our calendar-->
let startingDay = new Date(this.getFullYear(), this.getMonth(), 1).getDay();
...
let border = startingDay;
//For each day in the month, insert it into the calendar-->
for ( let id = '',  i = 1; i <= numberOfDays; i++ ) 
...
for ( let x = 0; x <= today.getMonthNames().length; x++ ) { selected_month += '<option value="' + today.getMonthNames()[x] + ' 1, ' + today.getFullYear() + '">' + today.getMonthNames()[x] + '</option>'; }
